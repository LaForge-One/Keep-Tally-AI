...
const STATUS_CONFIG: Record<string, { label: string; className: string }> = {
  ...
  sent: { label: "Sent to Warehouse", className: "bg-blue-100 text-blue-700 border-blue-200" },
  picked: { label: "Ready to Receive", className: "bg-amber-100 text-amber-700 border-amber-200" },
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <Badge className={`font-medium text-xs border hover:${cfg.className} ${cfg.className}`}>
      {cfg.label}
    </Badge>
  );
}
...
export default function OrdersPage() {
  ...
  const { data: orders = [], refetch } = useQuery<OrderSummary[]>({
  ...
  async function handleArchive(order: OrderSummary, e: React.MouseEvent) {
    ...
    try {
      const res = await fetch(`${BASE}/api/orders/${order.id}/archive`, { method: "POST" });
      ...
      toast({ title: "Error", description: "Could not archive order.", variant: "destructive" });
    } finally {
      setArchiving(null);
    }
  }
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`${BASE}/api/orders/${deleteTarget.id}`, { method: "DELETE" });
      if (!res.ok) {
        ...
        <div className="rounded-lg border border-border bg-card overflow-hidden shadow-sm">
          {orders.length === 0 ? (
            <div className="text-center py-16 px-4">
              ...
              <p className="text-sm text-muted-foreground mt-1">{emptyMessages[activeView]}</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/40 hover:bg-muted/40">
                  <TableHead className="pl-4 font-semibold text-foreground w-24">Order #</TableHead>
                  ...
                  return (
                    <TableRow
                      ...
                      className={`cursor-pointer hover:bg-muted/40 ${
                        isDeleted ? "opacity-50" : isArchived ? "opacity-70 bg-muted/20" : ""
                      }`}
                      onClick={() => !isDeleted && navigate(`/orders/${order.id}`)}
                    >
                        ...
                        {!isDeleted && (
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48">
                              {isArchived ? (
                                <DropdownMenuItem onClick={(e) => handleArchive(order, e)}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            <p className="text-sm text-muted-foreground">
              Select a location. The order will be auto-filled with all items currently below par level.
            </p>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateOrder} disabled={!newLocation || creating}>
      ...
      <Dialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete <strong>Pick List #{deleteTarget?.id}</strong> for{" "}
              <strong>{deleteTarget?.location}</strong>? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <div className="bg-destructive/10 border border-destructive/20 rounded-lg px-4 py-3 text-sm text-destructive">
            This pick list and all its line items will be permanently removed from active views. The action will be logged with your user, role, and timestamp.
          </div>
