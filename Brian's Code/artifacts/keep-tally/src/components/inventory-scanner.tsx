...
import { getListItemsQueryKey, getGetDashboardSummaryQueryKey } from "@workspace/api-client-react";
import {
  X,
  CameraOff,
  CheckCircle2,
...
export function InventoryScanner({ open, onClose, inventoryType = "store" }: InventoryScannerProps) {
  ...
  const handleScan = useCallback(async (code: string) => {
    ...
    try {
      if (inventoryType === "warehouse") {
        ...
        const res = await fetch(`${BASE}/api/items/barcode/${encodeURIComponent(code)}?${params}`);
        if (res.status === 404) {
          setResult({ found: false, storeItem: null, otherItems: [], warehouseItem: null });
          return;
        }
        if (!res.ok) throw new Error("Lookup failed");
        const data: BarcodeLookupResponse = await res.json();
        setResult({ found: data.found, storeItem: data.item, otherItems: [], warehouseItem: null });
  ...
  async function handleVerify() {
    ...
    try {
      ...
      toast({ title: diff === 0 ? "Count verified ✓" : `Updated (${diff > 0 ? "+" : ""}${diff})`, description: item.name });
      invalidateInventory();
      resetToCamera();
    } catch {
          ...
          <div className="relative flex-1 bg-black overflow-hidden">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              ...
              <div className="relative z-10 w-72 h-28">
                ...
                {/* Animated scan line */}
                {cameraActive && (
                  <div className="absolute left-2 right-2 top-0 h-0.5 bg-sky-400 shadow-[0_0_8px_2px_rgba(56,189,248,0.8)] animate-scan-line" />
                )}
              </div>
        ...
        <div className="flex flex-col h-full bg-background">
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-card shrink-0">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-muted-foreground font-medium">Scanned</p>
              <code className="text-sm font-mono font-semibold text-foreground truncate">{barcode}</code>
            </div>
            <button
              onClick={() => { stopCamera(); onClose(); }}
          ...
          <div className="flex-1 overflow-y-auto">
                  ...
                  <>
                    ...
                    {item && (
                      <div className="space-y-3">
                        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-xs text-emerald-700 font-semibold uppercase tracking-wide">Found · {item.location}</p>
                            <h2 className="font-bold text-lg leading-tight truncate">{item.name}</h2>
                            <p className="text-xs text-emerald-700/70">{item.category}</p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 divide-x divide-border rounded-2xl border border-border overflow-hidden bg-card">
                          ...
                          />
                          <Stat label="Par Level" value={item.parLevel} muted />
                          {otherItem ? (
                            <Stat label={otherItem.location} value={otherItem.quantity} blue />
                          ) : (
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {inventoryType === "store" && (
                            <ActionBtn
                              ...
                              label="Verify Count"
                              primary
                              onClick={() => { setCountedQty(""); setVerifyReason("adjustment"); setPhase("verify"); }}
                            />
                          )}
                          <ActionBtn
                            icon={ArrowRightLeft}
                            ...
                            onClick={() => { setAdjustQty(String(item.quantity)); setAdjustReason("adjustment"); setAdjustNotes(""); setPhase("adjust"); }}
                          />
                          <ActionBtn
                            icon={ShieldAlert}
                            label="Mark Theft"
                            onClick={() => { setAdjustQty(String(Math.max(0, item.quantity - 1))); setAdjustReason("theft"); setAdjustNotes(""); setPhase("adjust"); }}
                          />
                          <ActionBtn
                            icon={Flame}
                            label="Mark Spoilage"
                            onClick={() => { setAdjustQty(String(Math.max(0, item.quantity - 1))); setAdjustReason("spoilage"); setAdjustNotes(""); setPhase("adjust"); }}
                          />
                          <ActionBtn
                            icon={Wrench}
                            label="Mark Damaged"
                            onClick={() => { setAdjustQty(String(Math.max(0, item.quantity - 1))); setAdjustReason("damage"); setAdjustNotes(""); setPhase("adjust"); }}
                          />
                          <ActionBtn
                            icon={Package}
                            label="Return to WH"
                            onClick={() => { setAdjustQty("0"); setAdjustReason("return_to_warehouse"); setAdjustNotes(""); setPhase("adjust"); }}
                          />
                        </div>
                    ...
                    {!item && otherItem && (
                      <div className="space-y-3">
                        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-amber-700/80">
                              Found in {otherItem.location} · {otherItem.quantity} units available
                            </p>
                          </div>
                        </div>
                        <p className="text-sm text-muted-foreground px-1">
                          Would you like to add <strong>{otherItem.name}</strong> to{" "}
                        ...
                        <button onClick={resetToCamera} className="w-full text-sm text-muted-foreground py-2 hover:text-foreground transition-colors flex items-center justify-center gap-2">
                          <RotateCcw className="w-4 h-4" /> Scan another item
                        </button>
                      </div>
                    )}
                          ...
                          <>
                            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 flex items-start gap-3">
                              ...
                              <div className="min-w-0">
                                <p className="font-bold text-base text-blue-900">{result.warehouseItem.name}</p>
                                <p className="text-sm text-blue-700">{result.warehouseItem.category}</p>
                                <p className="text-xs text-blue-600 mt-1">
                                  Found in warehouse catalog &mdash; {result.warehouseItem.quantity} units in stock
                                </p>
                              </div>
                            </div>
                            <Button
                              ...
                              onClick={() => {
                                ...
                                setCreateCategory(result.warehouseItem!.category);
                                setCreateQty("0");
                                setCreatePar(String(result.warehouseItem!.minPar || 0));
                                setPhase("create");
                              }}
                            >
                              <PackagePlus className="w-5 h-5 mr-2" />
                          ...
                          <>
                            <div className="bg-muted border-2 border-dashed border-border rounded-2xl p-4 flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">This barcode isn't in your inventory or warehouse catalog.</p>
                              </div>
                            </div>
                            <Button
                              className="w-full h-12 font-bold"
            ...
            {phase === "verify" && item && (
              <div className="p-4 space-y-4 max-w-lg mx-auto">
                ...
                <div className="bg-card border border-border rounded-2xl p-4 space-y-4">
                  ...
                  {countedQty !== "" && parseInt(countedQty) !== item.quantity && (
                    <div>
                      ...
                      <ReasonGrid value={verifyReason} onChange={setVerifyReason} />
                    </div>
                  )}
                  {countedQty !== "" && parseInt(countedQty) === item.quantity && (
                ...
                <Button className="w-full h-14 text-base font-bold" disabled={countedQty === "" || submitting} onClick={handleVerify}>
                  <CheckCircle2 className="w-5 h-5 mr-2" />
                  {parseInt(countedQty) === item.quantity ? "Confirm Count" : "Save & Update"}
                </Button>
              </div>
            )}
            ...
            {phase === "adjust" && item && (
              <div className="p-4 space-y-4 max-w-lg mx-auto">
                <ItemHeader item={item} label={
                  adjustReason === "theft" ? "Marking theft" :
                  adjustReason === "spoilage" ? "Marking spoilage" :
                  adjustReason === "damage" ? "Marking damage" :
                  adjustReason === "return_to_warehouse" ? "Return to warehouse" :
                  "Adjusting quantity"
                } />
                <div className="bg-card border border-border rounded-2xl p-4 space-y-4">
                  <div>
                  ...
                  <div>
                    <label className="text-sm font-semibold block mb-2">Reason</label>
                    <ReasonGrid value={adjustReason} onChange={setAdjustReason} />
                  </div>
                  <div>
                ...
                <Button className="w-full h-14 text-base font-bold" disabled={adjustQty === "" || submitting} onClick={handleAdjust}>
                  Save Adjustment
                </Button>
              </div>
            )}
            ...
            {phase === "add-to-store" && otherItem && (
              <div className="p-4 space-y-4 max-w-lg mx-auto">
                ...
                <div className="bg-card border border-border rounded-2xl p-4 space-y-4">
                  ...
                  <div>
                    <label className="text-sm font-semibold block mb-1.5">Par Level</label>
                    <Input type="number" min={0} value={addPar} onChange={(e) => setAddPar(e.target.value)}
                      className="h-11 rounded-xl" placeholder={`Default: ${otherItem.parLevel}`} />
                  </div>
                  <div>
                    ...
                    <Input value={addCategory} onChange={(e) => setAddCategory(e.target.value)}
                      className="h-11 rounded-xl" placeholder={otherItem.category} />
                  </div>
                </div>
                <Button className="w-full h-14 text-base font-bold" disabled={submitting} onClick={handleAddToStore}>
                  <PackagePlus className="w-5 h-5 mr-2" />
                  Add to {selectedLocation ?? "Store"}
                </Button>
              </div>
            ...
            {phase === "create" && (
              <div className="p-4 space-y-4 max-w-lg mx-auto">
                <div className="bg-card border border-border rounded-2xl p-4">
                  ...
                  <code className="text-sm font-mono text-muted-foreground">{barcode}</code>
                </div>
                <div className="bg-card border border-border rounded-2xl p-4 space-y-4">
                  <div>
                    ...
                    <Input value={createName} onChange={(e) => setCreateName(e.target.value)}
                      placeholder="e.g. Coke Zero 20 oz" className="h-11 rounded-xl" autoFocus />
                  </div>
                  <div>
                    <label className="text-sm font-semibold block mb-1.5">Category</label>
                    <Input value={createCategory} onChange={(e) => setCreateCategory(e.target.value)}
                      placeholder="e.g. Soda, Snacks" className="h-11 rounded-xl" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-sm font-semibold block mb-1.5">Quantity</label>
                      <Input type="number" min={0} value={createQty} onChange={(e) => setCreateQty(e.target.value)}
                        className="h-11 rounded-xl text-center font-bold" />
                    </div>
                    <div>
                      <label className="text-sm font-semibold block mb-1.5">Par Level</label>
                      <Input type="number" min={0} value={createPar} onChange={(e) => setCreatePar(e.target.value)}
                        className="h-11 rounded-xl text-center font-bold" />
                    </div>
                  </div>
                  <div>
                ...
                <Button className="w-full h-14 text-base font-bold" disabled={!createName.trim() || submitting} onClick={handleCreate}>
                  <PackagePlus className="w-5 h-5 mr-2" />
                  Create Item
                </Button>
              </div>
...
function Stat({ label, value, alert, muted, blue }: { label: string; value: number | string; alert?: boolean; muted?: boolean; blue?: boolean }) {
  return (
    <div className="p-3 text-center">
      <p className={`text-2xl font-black tabular-nums ${alert ? "text-red-600" : muted ? "text-muted-foreground" : blue ? "text-blue-600" : "text-foreground"}`}>
        {value}
      </p>
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-4">
      ...
      <p className="text-sm text-muted-foreground mt-0.5">
        {item.location} · Current: <strong className="text-foreground">{item.quantity}</strong> · Par: <strong className="text-foreground">{item.parLevel}</strong>
      </p>
    </div>
  );
}

function QtySpinner({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
...
function ActionBtn({ icon: Icon, label, primary, onClick }: {
  ...
  return (
    ...
    >
      <Icon className={`w-4 h-4 shrink-0 ${primary ? "text-primary-foreground" : "text-muted-foreground"}`} />
      {label}
    </button>
  );
}
